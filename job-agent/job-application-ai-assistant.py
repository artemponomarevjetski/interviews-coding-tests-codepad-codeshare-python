...
from batch_import_skill import batch_import_jobs_from_text
from export_json_skill import export_jobs_json
from auto_draft_skill import auto_draft_all_p1_jobs
...
def command_loop() -> None:
    ...
    while True:
        try:
            ...
            if raw == "/list-remote":
                rows = list_jobs("Remote", limit=100)
                if not rows:
                ...
                for r in rows:
                    print(
                        f"#{r['id']:>3} P{r['priority']} {r['status']:<16} "
                        f"{r['company'][:20]:<20} | {r['title'][:45]:<45} | "
                        f"job_id={r['job_id']:<10} | {r['location']}"
                    )
                continue
            ...
            if raw.startswith("/export-json"):
                parts = shlex.split(raw)
                path = Path(parts[1]) if len(parts) > 1 else OUTPUT_DIR / f"jobs_export_{today()}.json"
                export_jobs_json(path, list_jobs, init_db)
                continue
            ...
            if raw == "/batch-import":
                print("Paste job data (JSON, SQL INSERT, CSV, or key:value blocks).")
                ...
                while True:
                    ...
                    lines.append(line)
                text = "\n".join(lines)
                count = batch_import_jobs_from_text(text)
                print(f"✅ Batch import complete. Inserted {count} jobs.")
                continue
            ...
            if raw == "/draft-all":
                auto_draft_all_p1_jobs(ask_llm, list_jobs)
                continue
